...
class sc_sis():
    ...
    def runOneStep(self, x0, x1, x2, incMatrix_AdjAct1,incMatrix_AdjAct2, network):
        self.network = network
        self.num_nodes = network.num_nodes
        self.betaList = (self.eff_infection*self.mu/network.avg_k).to(self.device)
        self.adjActEdges,self.adjActTriangles = self.getAdjActivateSimplex(incMatrix_AdjAct1, incMatrix_AdjAct2)
        x0, x1, x2, new_x0, new_x1, new_x2, y_true = self.spread(x0, x1, x2, self.adjActEdges,self.adjActTriangles)
        return x0, x1, x2, new_x0, new_x1, new_x2, y_true, self.adjActEdges, self.adjActTriangles

    def spread(self, x0, x1, x2, adjActEdges,adjActTriangles):
        old_x0 = copy.deepcopy(x0)
        old_x1 = copy.deepcopy(x1)
        old_x2 = copy.deepcopy(x2)
        ...
        inf_S_index = self.dy_action_infection(x0, x1, x2, s_index, adjActEdges, adjActTriangles, true_tp)
        recover_I_index = self.dy_action_recovery(I_index, true_tp)
        new_x0 , new_x1, new_x2 = self.update_x0(x0, inf_S_index, recover_I_index)
        return old_x0, old_x1, old_x2, new_x0, new_x1, new_x2, true_tp
    ...
    def dy_action_infection(self, x0, x1, x2, s_index, adjActEdges, adjActTriangles, true_tp):
        # 感染概率
        betaList = self.betaList.view(-1, 1).repeat(1, self.num_nodes)
        adjActSimplex = torch.stack((adjActEdges,adjActTriangles),dim=0)
        # 不被感染的概率矩阵
        NIP_Matrix:'maxDimension,num_nodes' = torch.pow(1 - betaList, adjActSimplex)
    ...
    def getAdjActivateSimplex(self, incMatrix_AdjAct1, incMatrix_AdjAct2):
        '''
        聚合邻居单纯形信息
        了解节点周围单纯形，【非激活态，激活态】数量
        '''
        adjActEdges = torch.sparse.sum(incMatrix_AdjAct1,dim=1).to_dense()
        adjActTriangles = torch.sparse.sum(incMatrix_AdjAct2,dim=1).to_dense()

        return adjActEdges,adjActTriangles
