...
class SATLayer_regular(nn.Module):
    ...
    def __init__(self, input_size, output_size, bias=True):
        ...
        self.linear_out = nn.Linear(2*input_size, output_size, bias=bias)
        self.LinearAgg = nn.Linear(2*output_size, output_size, bias=bias)
        self.leakyrelu = nn.LeakyReLU(0.2)
        self.relu = nn.ReLU()

    def attention_agg(self, xi, xj, inc_matrix_adj):
    ...
    def forward(self, x0, x1, x2, network,dynamics):
        """
...
class SimplexAttentionLayer(nn.Module):
    ...
    def forward(self, x0, x1, x2, network,dynamics):
        x0_1 = torch.stack([sat(x0, x1, x2, network,dynamics) for sat in self.layer0_1])
