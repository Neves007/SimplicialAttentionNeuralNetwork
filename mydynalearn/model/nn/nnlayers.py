...
def get_triangle_in_layers(config):
    in_channels = config.in_channels
    device = config.device
    in_layer = inLinearLayers(in_channels).to(device)
    return in_layer

def get_gat_layer(config):
    in_features = config.gnn_channels
    out_features = config.gnn_channels
    concat = config.concat
    heads = config.heads
    device = config.device
    gat_layer = GraphAttentionLayer(in_features, out_features, heads, concat).to(device)
    return gat_layer
def get_sat_layer(config):
    in_features = config.gnn_channels
    out_features = config.gnn_channels
    concat = config.concat
    heads = config.heads
