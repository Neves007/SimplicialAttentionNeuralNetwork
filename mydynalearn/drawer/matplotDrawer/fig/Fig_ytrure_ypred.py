...
class Fig_ytrure_ypred():
    ...
    def _get_performance_data(self, x, predict_TP, y_ob, true_TP):
        ...
        with torch.no_grad():
            ...
            S_S = torch.where((x[:, self.statesMap["S"]] == 1) & (y_ob[:,self.statesMap["S"]]==1))[0]
            S_I = torch.where((x[:, self.statesMap["S"]] == 1) & (y_ob[:,self.statesMap["I"]]==1))[0]
            I_S = torch.where((x[:, self.statesMap["I"]] == 1) & (y_ob[:,self.statesMap["S"]]==1))[0]
            I_I = torch.where((x[:, self.statesMap["I"]] == 1) & (y_ob[:,self.statesMap["I"]]==1))[0]
            # 混淆矩阵
