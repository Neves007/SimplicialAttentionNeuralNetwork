...
class VisdomDrawer:
    ...
    def get_performance_data(self,x,predict_TP, y_ob, fun_TP):
        ...
        with torch.no_grad():
            ...
            S_S = torch.where((x[:, self.statesMap["S"]] == 1) & (ob_labels == 0))[0]
            S_I = torch.where((x[:, self.statesMap["S"]] == 1) & (ob_labels == 1))[0]
            I_S = torch.where((x[:, self.statesMap["I"]] == 1) & (ob_labels == 0))[0]
            I_I = torch.where((x[:, self.statesMap["I"]] == 1) & (ob_labels == 1))[0]
            # 混淆矩阵
    ...
    def draw_performance(self,x,predict_TP, y_ob, fun_TP):
        ...
        opts = dict(title='Train performace, r = {:0.4f}'.format(corrcoef),
        ...
        self.wind.scatter(X=X,Y=Y, win='Train performace', opts=opts)

        # opts['markersymbol'] = "star"
        # opts['markercolor'] = green
        # self.wind.scatter(X=S_I_X, win='Train performace', opts=opts)
        # self.wind.scatter(X=I_S_X, win='Train performace', opts=opts)
        # self.wind.scatter(X=I_I_X, win='Train performace', opts=opts)
